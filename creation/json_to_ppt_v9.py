...
class PPTCreatorApp:
    ...
    def add_title_slide(self, title):
        ...
        slide.shapes.placeholders[1].text = "Github Interview Questions - 4"
    ...
    def load_json(self):
        input_dir = filedialog.askdirectory(title="Select Folder with JSON Files")
        if not input_dir:
            return

        output_dir = filedialog.askdirectory(title="Select Folder to Save PPT Files")
        if not output_dir:
            return
        ...
        for file_name in json_files:
            ...
            ppt_filename = os.path.splitext(file_name)[0] + ".pptx"
            ppt_path = os.path.join(output_dir, ppt_filename)
            self.prs.save(ppt_path)

        self.status_label.config(text=f"✅ All PPTs generated in: {output_dir}", fg="blue")
